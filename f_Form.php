...
function pulldownDate_set($type,$beforeyear,$afteryear,$name,$over,$post,$ReadOnly,$formName,$isnotnull){
	...
	else
	{
		...
		for ($countYear1=$beforeyear; $countYear1 <= $year; $countYear1++)
		{
			...
			else
			{
				...
				{
					//↓↓↓　令和対応修正
					$temp_d_value = $d_value;
					if($temp_d_value === '')
					{
						$temp_d_value = '1';
					}
					if($m_value < $date[1] || ($m_value == $date[1] && $temp_d_value < $date[2]))
					//↑↑↑　令和対応修正
					{
	...
	if($select_year != "")
	{
		...
		for($nenngou_count = 0; $nenngou_count < count($start_array) ; $nenngou_count++ )
		{
			...
			{
				//↓↓↓　令和対応修正
				$wareki_y_value = $y_value;
				if ( is_numeric($wareki_y_value) === true && $type == 1 )
				{
					//$y_valueに西暦が入っているのでうまくいかない。月日を含めて和暦の年に変換する
					$wareki_y_value = wareki_year_by_date($y_value, $m_value, $d_value)."年";
				}
				if($wareki."年" == $wareki_y_value)
				//↑↑↑　令和対応修正
				{
		...
		if($form_format_type == 9)
		{
			...
			for($k = 0 ; $k < $form_num ; $k++)
			{
				...
				else
				{
					...
					$check_column_str .= $form_name."~".$form_length."~".$form_format."~".false.",";
				}
...
戻り値	入力フォームhtml
************************************************************************************************************/
function wareki_year_by_date($year, $month, $day){
	//------------------------//
	//        初期設定        //
	//------------------------//
	$item_ini = parse_ini_file('./ini/item.ini', true);													// form.ini呼び出し
	...
	{
		if( strtotime($target_date) >= strtotime($start_array[$i]) )
		{
